...
export default function DashboardPage() {
  ...
  return <ProtectedShell admin={user?.role === 'ADMIN'}><div className="page-wrap">
    ...
    <div className="dashboard-heading"><div><h1>{user?.name ? `Welcome, ${user.name.split(' ')[0]}.` : 'Your research workspace.'}</h1><p>Questions, evidence and reasoning that belong to your account.</p></div><span className="role-pill">{user?.role ?? 'USER'}</span></div>
