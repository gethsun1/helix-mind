...
type Edge = { id: string; source: string; target: string; relationship: string; stance: string; provenance?: Record<string, unknown> | null; claims: { id: string; text: string; paperId: string; paperTitle: string | null }[] };
...
export default function KnowledgePage() {
  ...
  return <ProtectedPage eyebrow="RESEARCH / KNOWLEDGE GRAPH" title="Knowledge graph.">
    ...
    {error ? <div className="panel"><p className="error-text">{error}</p></div> : loading ? <div className="panel"><p>Loading knowledge…</p></div> : !selectedInvestigation ? <div className="panel empty-state"><span>∴</span><h3>Choose an investigation.</h3><p>Knowledge is scoped to the investigations you are authorized to access.</p></div> : !summary || summary.graph.nodes.length === 0 ? <div className="panel empty-state"><span>∴</span><h3>No source-grounded knowledge yet.</h3><p>Run an investigation with retrieved abstracts to create auditable entities, propositions, and evidence.</p></div> : <>
